%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  MORPHOLOGICAL ANALYSER  - convert words into root form to be identified by Word Net
...
morph_tokens([w(Chars)|Tokens],Morphs) :-    	% input = list of token(s)
   !,
   morph(Chars,Rest,Morphs),    
   morph_tokens(Tokens,Rest).
morph_tokens([_|Tokens],Morphs) :-           	% skip numeric or special-character token
   morph_tokens(Tokens,Morphs).
morph_tokens(Token,Morphs) :-                	% input = single token
   \+ is_list(Token),
   morph_tokens([Token],Morphs).
morph_tokens([],[]).				% input = [] finished!
						%  Same as morph_tokens/2 except that it returns every alternative analysis in a list of lists - (i.e. )[[[[testing]], [[teste, -ing]], [[test, -ing]]], [[[more]]]] 
morph_tokens_bag(Token,List) :-
   \+ is_list(Token),
   findall(Alternative, morph_tokens(Token,Alternative), List).
morph_tokens_bag([First|RestTokens],[List|RestList]) :-
   findall(Alternative, morph_tokens(First,Alternative), List),
   morph_tokens_bag(RestTokens,RestList).
morph_tokens_bag([],[]).
%%-----------------------------------------------
						%  Converts an atom to a list of morphemes (i.e. [testing,one,two,three] --> [[test,-ing],[one],[two],[three]]
morph_atoms([AtomWord|Rest],List) :-	   	% Input = list of atom(s)
   atom_chars(AtomWord,RawList),
   morph(RawList,RestResult,List),
   morph_atoms(Rest,RestResult).
morph_atoms(SingleAtom,List) :-            	% Input = single atom
   \+ is_list(SingleAtom),
   morph_atoms([SingleAtom],List).	
morph_atoms([],[]).				% input = [] finished!
						%  Same as morph_atoms/2 except that it returns evey alternative analysis in a list of lists e.g. [[[[testing]], [[teste, -ing]], [[test, -ing]]], [[[more]]]]
morph_atoms_bag(Token,List) :-
   \+ is_list(Token),
   findall(Alternative, morph_atoms(Token,Alternative), List).
morph_atoms_bag([First|RestTokens],[List|RestList]) :-
    findall(Alternative,morph_atoms(First,Alternative),List),
    morph_atoms_bag(RestTokens,RestList).
morph_atoms_bag([],[]).
%%-----------------------------------------------
						%  Converts a list of characters to a list of morphemes.  (i.e. [[r,u,n,n,i,n,g],[f,a,s,t,e,r]] --> [[run,-ing],[fast,-er]]
morph_chars([Chars|Rest],List) :-          	% Input = list of character lists
   is_list(Chars),
   morph(Chars,RestResult,List),
   morph_chars(Rest,RestResult).
morph_chars([Chars|Rest],List) :-      	    	% Input = single list of characters
   \+ is_list(Chars),
   morph_chars([[Chars|Rest]],List).
morph_chars([],[]).				% input = [] finished!
...
morph_chars_bag([First|RestTokens],[List|RestList]) :-
    findall(Alternatives, morph_chars(First,Alternatives), List),
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Simplified morphological analyzer that only removes suffixes which contain vowels, such as -es, -ed, -ing.
morph([], []) :- 
	!.
morph(Chars, Root) :- 
	split_suffix(Chars, Root).
morph([H|T], [H|T2]) :- 
	morph(T, T2).			

split_suffix([V,C,C,V2|Rest],[V,C]) :-
	vowel(V), 
	\+ vowel(C), 
	vowel(V2), 
	suffix([V2|Rest]).
split_suffix([s,h,e,s],[s,h]).
split_suffix([c,h,e,s],[c,h]).
split_suffix([s,e,s],[s]).
split_suffix([z,e,s],[z]).
split_suffix([x,e,s],[x]).
split_suffix([F|R], [F]) :- 
	suffix(R).
split_suffix( X , [] , S ):-
	suffix(	X , S ).
